 ...
 * Adapted for VibeBooks PRO (Lead Architect: นายเกียรติภูมิ หารศรีนาถ 64332110242-2)
 * A scroll-driven camera through live type.
 */
import { useId, useEffect, useLayoutEffect, useRef, type CSSProperties, type ReactNode } from "react";

// Isomorphic layout effect to avoid Next.js SSR hydration warnings
...
const DEFAULT_FONT = '"Arial Black", "Impact", "Arial", sans-serif';
...
export default function GlyphPortal({
  word = "VIBEBOOKS", focusChar = "B", interactive = true, background, front, children, scrollLength = 2.4,
  fontFamily = DEFAULT_FONT, fontWeight = 900, annotations = false,
  enterLabel = "เข้าสู่คลังหนังสือ (Enter Catalog)", className, style, onProgress,
}: GlyphPortalProps) {
  ...
  const text = word.trim().normalize("NFC") || "VIBEBOOKS";
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      ...
      <style>{`
        ${q}{--gp-paper:#08070e;--gp-ink:#ffffff;--gp-field:#0f0c1c;--gp-foreground:#f8fafc;position:relative;isolation:isolate;background:var(--gp-paper);color:var(--gp-ink);font-family:system-ui,-apple-system,sans-serif;}
        ...
        ${q} [data-gp-marks]{fill:none;stroke:#c5a880;opacity:.8;}
        ...
        ${q} [data-gp-letter]:focus-visible{outline:2px solid #06b6d4;outline-offset:5px;}
        ...
        ${q} [data-gp-select]{min-height:44px;min-width:90px;border:1px solid rgba(255,255,255,0.2);border-radius:12px;background:rgba(20,18,29,0.9);color:#ffffff;padding:0 12px;font:inherit;}
        ${q} [data-gp-select]:focus-visible{outline:2px solid #06b6d4;outline-offset:4px;}
        ...
        ${q} [data-gp-caption]{position:absolute;inset:auto 8% 9%;display:flex;align-items:center;justify-content:space-between;gap:1rem;font:12px/1.4 system-ui,sans-serif;opacity:var(--gp-caption,1);pointer-events:var(--gp-caption-hit,auto);z-index:30;}
        ${q} [data-gp-caption]{font-family:inherit;}
        ${q} [data-gp-front]{position:absolute;inset:0;opacity:var(--gp-caption,1);pointer-events:none;z-index:25;}
        ${q} [data-gp-front] a,${q} [data-gp-front] button{pointer-events:var(--gp-caption-hit,auto);}
        ${q} [data-gp-front]:focus-within{opacity:1;}
        ${q} [data-gp-hint]{max-width:32ch;color:rgba(255,255,255,0.7);font-weight:500;}
        ${q} [data-gp-enter]{display:inline-flex;align-items:center;gap:10px;min-height:44px;color:#ffffff;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);padding:0 18px;border-radius:9999px;font:inherit;font-weight:600;text-decoration:none;backdrop-filter:blur(8px);transition:all 200ms ease;}
        ${q} [data-gp-enter]:hover{background:rgba(255,255,255,0.18);border-color:rgba(76,215,246,0.5);transform:translateY(-1px);}
        ${q} [data-gp-enter]:focus-visible{outline:2px solid #06b6d4;outline-offset:5px;}
        ${q} [data-gp-caption]:focus-within{opacity:1;pointer-events:auto;}
        ${q} [data-gp-content]{box-sizing:border-box;position:relative;min-height:var(--gp-height,100svh);padding:clamp(24px,5%,64px);display:grid;align-content:center;color:var(--gp-foreground);background:var(--gp-field);overflow-wrap:anywhere;}
        ...
        ${q} [data-gp-default-title]{color:inherit;font:700 clamp(28px,4vw,56px)/1.1 system-ui,-apple-system,sans-serif;letter-spacing:-.02em;margin:0 0 16px;text-wrap:balance;}
        ${q} [data-gp-default-copy]{color:rgba(255,255,255,0.7);font:15px/1.6 system-ui,-apple-system,sans-serif;max-width:48ch;margin:0;}
      ...
      <div data-gp-pin>
          ...
          {background ?? (
            <div
              ...
              style={{
                ...
                background:
                  "radial-gradient(circle at 20% 15%, rgba(168,85,247,0.35), transparent 45%), radial-gradient(circle at 80% 25%, rgba(6,182,212,0.35), transparent 40%), radial-gradient(circle at 50% 80%, rgba(197,168,128,0.25), transparent 45%), linear-gradient(145deg, #090713 0%, #131024 50%, #080611 100%)",
              }}
            />
          )}
        </div>
        ...
        <label data-gp-touch-picker><span style={{ position: "absolute", width: 1, height: 1, overflow: "hidden", clipPath: "inset(50%)" }}>Entry letter</span><select data-gp-select defaultValue="">
          <option value="" disabled>เลือกตัวอักษรเพื่อเข้าสู่เว็บ</option>
        ...
        <div data-gp-caption>
          <span data-gp-hint aria-hidden="true">{interactive ? "เลื่อนหน้าจอเพื่อเข้าสู่ร้านค้า (Scroll to enter)" : annotations ? "A passage through live type" : ""}</span>
          <a data-gp-enter href={`#${uid}-content`}>{enterLabel}<span aria-hidden="true">↘</span></a>
        </div>
      </div>
      <div data-gp-content id={`${uid}-content`} tabIndex={-1}>
        {children ?? <div><h2 data-gp-default-title>VibeBooks Engineering & AI Series</h2><p data-gp-default-copy>คลังคู่มือสถาปัตยกรรมซอฟต์แวร์ระดับโปรดักชัน ภาษาไทยฉบับสมบูรณ์ (DEMO ONLY)</p></div>}
      </div>
    </section>
  );
}

export const Component = GlyphPortal;
